              ...
              <div class="col-sm-12 col-md-12">
                <div class="card">
                    <div class="card-header">
                      <i class="fa fa-align-justify"></i>{{ __('Lista Solicitudes De Actualizacion Proveedores a Risk') }}
                    ...
                    <div class="card-body">
                        <table class="table table-striped table-bordered" id="requests_risk-update-table">
                        <thead>
                          <tr>
                            <th>id</th>
                            <th>Ver</th>
                        ...
                        <tbody>
                          @foreach($requests_risk_updates as $value)
                            <tr>
                              <td>{{ $value->id}}</td>
                              <td>
                                <a href="{{ url('/genfar-update/' . $value->id) }}" class="btn btn-block btn-primary">Ver</a>
                              </td>
                              <td>{{ $value->solicitante_name}}</td>
                              <td>{{ $value->solicitante_email}}</td>
                              <td>{{ date('d-m-y', strtotime($value->date_fill))}}</td>
                            </tr>
                          @endforeach
                        </tbody>
                      </table>
...
<script type="text/javascript">
  $(document).ready(function() {
    $('#requests_risk-update-table').DataTable({
